...
## 버전 2.3.2 - 2025-10-16 (E2E 트리아지 진행 및 인증 관련 임시 수정)

### ✅ 변경 사항

- `tests/global-setup.ts`의 저장된 `storageState` 사용 로직을 개선: 기존에 저장된 토큰을 서버의 `/api/auth/me`로 검증하고, 유효하지 않으면 API 기반 재생성 또는 UI 기반 재생성으로 안전하게 대체합니다. (`E2E_FORCE_REGENERATE` 환경 변수로 강제 재생성 가능)
- `server/routes.ts`에 E2E 디버그 로그(조건부)를 추가하여 `/api/auth/register`, `/api/auth/login`, `/api/auth/me` 호출 시 최소한의 디버그 정보를 출력하도록 했습니다(환경변수 `E2E_DEBUG_AUTH=1` 또는 `NODE_ENV!=="production"`에서 활성화).
- `client/src/lib/http.ts`의 HTTP 래퍼를 수정하여 `403 Forbidden` 응답을 인증 실패(로그아웃)로 처리하지 않도록 변경했습니다. 401 응답만 인증 실패로 보고 토큰 제거 및 로그인 페이지로 리디렉션합니다.

### 🧪 검증 및 결과

- 디버그 스크립트(`scripts/debug-login.mjs`)로 수동 검증을 수행했습니다: `/api/auth/register` → `/api/auth/login` → `/api/auth/me` 흐름에서 서버는 토큰을 정상 발급 및 검증했으나, 클라이언트가 403 응답을 받아 토큰을 제거하면서 로그인 상태를 잃는 현상을 확인했습니다. 해당 클라이언트 로직을 수정해 이 문제를 방지했습니다.
- 변경사항 커밋 후 lint, TypeScript 체크, 및 서브셋 스모크 테스트를 실행했으며 모두 통과했습니다.

### 🔧 후속 권장 작업

1. Authentication + Wiki E2E 그룹을 재실행하여 토큰 유지/리다이렉트 이슈가 해결됐는지 확인하세요.
2. `E2E_DEBUG_AUTH`를 CI 디버그 시나리오에서만 활성화하고, 프로덕션에서는 비활성화 상태로 유지하세요.
3. 장기적으로는 E2E에서의 `403` 응답 패턴을 줄이도록 권한 처리 흐름(특히 테스트용 계정 권한)을 조정하는 것이 좋습니다.
