...
- **테스트 서버 격리**: `playwright.config.ts`에 `reuseExistingServer: false` 옵션을 추가하여, 매 테스트 실행 시 새로운 테스트 서버를 사용하도록 보장함으로써 테스트 간의 상태 공유로 인한 잠재적 충돌을 방지했습니다.
...
### 💡 개발자 인사이트
- **안정적인 아키텍처의 중요성**: 이번 리팩토링을 통해 서버의 구조를 더 단순하고 명확하게 만들어 향후 유지보수성과 안정성을 높이는 기반을 마련했습니다.
