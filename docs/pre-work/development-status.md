...
## 📅 최종 업데이트: 2025-09-29 (E2E 안정화: AI 모의, 셀렉터 보강)
...
- **E2E 안정화 (진행 중)**
  - 서버: `MOCK_AI` 모드 도입으로 `/api/ai/*` 엔드포인트의 외부 의존성 제거 (테스트/무키 환경 자동 모의)
  - 클라이언트: `ai-search` 결과 JSON 파싱 수정, Dashboard/Tasks 헤더 상시 렌더 + 인라인 로딩 전환, Tasks의 Select 트리거에 `aria-label` 추가
  - 테스트: 대시보드 단언을 텍스트 기반으로 조정, 과제 트래커의 Select 단언을 역할 기반으로 교정, AI 검색은 모의 or 유효 키 조건부 실행
  - 결과: 캘린더/데이터베이스/AI 검색 부분 PASS, 대시보드/과제는 후속 재실행 필요(셀렉터 반영 후 확인)
