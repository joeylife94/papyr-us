...
### 2025-09-22 — QA/CI 개선 사항 반영

- Playwright의 HTML 리포트 및 아티팩트 업로드가 CI에 추가되어, E2E 실패 시 분석 자료 확보가 용이해졌습니다.
- 테스트 안정화 노력(타임아웃 증가, 테스트 격리, UI 기반 리팩토링)이 계속 진행 중입니다.

## Next steps

- E2E 테스트의 flaky 케이스를 정리하고, 우선순위에 따라 리팩토링 및 보완 테스트를 진행하세요.
- 성능 분석을 통해 테스트 환경에서의 응답 지연 원인을 파악하고 최적화하세요(쿼리 튜닝, 인덱스 검토 등).
- 테스트 실행 시간과 실패 빈도를 모니터링하는 자동화 대시보드를 도입하면 안정성 추적에 도움이 됩니다.

## History

### 2025-10-02 — 로컬/도커 환경 정비 및 E2E 트리아지 시작

- 오늘 한 일 요약:
  - `node_modules` 재설치(EPERM 문제 해결), 개발 서버 포트(5001) 충돌 해결, Docker로 Postgres+앱 컨테이너 기동 후 API 정상 응답 확인.
  - Playwright 전체 E2E 실행으로 다수 실패와 trace 아티팩트 수집(주로 요소 탐색 타임아웃).
  - `playwright.config.ts`와 `package.json`의 E2E 스크립트를 업데이트하여 존재하는 `tests/storageState.json`를 자동으로 사용하고, `E2E_USE_STORAGE_STATE=1`을 기본 실행 플래그로 설정함.

- 권장 다음 작업:
  1. 대표 실패 2~3건을 trace viewer로 분석하여 근본 원인(인증/데이터/타이밍)을 확정.
  2. storageState 생성/재사용이 불안정하다면 global-setup 로그인 로직 보강.
  3. 시드 데이터가 부족하면 seed 스크립트 보강 후 재실행.
