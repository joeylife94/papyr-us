...
2. 선택 영역 위에 자동으로 나타나는 인라인 툴바에서 액션 선택:
   - **Summarize** — 선택 내용을 1-3문장으로 요약하여 대체
   - **Rewrite** — 더 명확하고 전문적인 문체로 재작성 (의미 보존)
   - **Taskify** — 선택 텍스트를 `- ` 형식의 할 일 목록으로 변환
3. AI가 처리한 결과로 선택 텍스트가 즉시 대체됨

> **요구사항**: OpenAI API 키(`OPENAI_API_KEY`) 및 `FEATURE_AI_SEARCH=true` 환경 변수 설정 필요
...
- 문서 요약: 조리 있는 문단을 선택하고 Summarize 클릭
- 문체 재작성: 불명확한 문장을 선택하고 Rewrite 클릭
- 할 일 추출: 회의록 텍스트를 선택하고 Taskify 클릭
