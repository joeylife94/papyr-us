...
> **최종 업데이트**: 2026-03-31  
> TL;DR: Papyr.us는 React 18 + Express.js 기반의 노션급 팀 협업 위키 플랫폼으로, 24종 블록 에디터, Yjs CRDT 실시간 협업, GPT-4o AI Copilot, Notion-style 데이터베이스, 워크플로우 자동화, 페이지 분석/즐겨찾기/활동 피드/내보내기, 휴지통(soft delete)/일괄 작업/페이지 복제/대시보드 통계를 제공합니다. TypeScript ~35,000줄, 160+ API, 22개 DB 테이블, 94개 React 컴포넌트.
...
- **인라인 AI** — 에디터 선택 툴바 활성 AI 편집 경로: Summarize / Rewrite / Taskify (`POST /api/ai/inline`)
- **AI Writing Assistant** — continue, improve, summarize, translate, fixGrammar, shorten, lengthen 7가지 명령 (`POST /api/ai/assist`); 에디터 툴바에 마운트되지 않음 (비활성 경로)
...
- 액션: send_notification, create_task, add_tag 등; 외부 연결 액션으로 **webhook**, **slack_webhook**, **send_email** 지원
  - `send_email`: SMTP(`EMAIL_HOST`/`EMAIL_USER`/`EMAIL_PASS`) 설정 시 nodemailer SMTP 발송; SMTP 미설정 시 숫자 사용자 ID 수신자에한 인앱 알림 폴백만 동작 (이메일 주소 문자열은 사용자로 해석되지 않음)
...
- **Soft Delete / 휴지통**: 페이지 삭제 시 즉시 제거 대신 `deletedAt` 타임스탬프 설정, 복원(restore) 지원, 관리자 전용 전체 비우기
- **페이지 복제**: 원클릭 페이지 복제 (제목/내용/태그/블록 복사, 새 slug 자동 생성)
- **일괄 작업 API**: 여러 페이지를 한번에 폴더 이동, 태그 추가/제거, 삭제
- **대시보드 통계 API**: 페이지/태스크/댓글 수, 완료율, 최근 페이지, 주간 성장률 제공
...
### 2026-03-31 — 보안 강화 & 포트폴리오 기능 추가 (v1.2)

**보안 수정**

- SQL Injection 취약점 수정: `sql.raw()`를 parameterized `MAKE_INTERVAL(days => ${days})`로 교체
- 비밀번호 정책 강화: 영문 + 숫자 + 특수문자 필수
- bcrypt 해싱 일관성: 회원가입/비밀번호 변경 모두 salt rounds 12로 통일
- AI 엔드포인트 Rate Limiting 추가 (15 req/min) — 비용 폭주 방지
- `/api/pages/:id/view` 조회 기록 Rate Limiting 추가 — 분석 스팸 방지

**신규 기능**

- Soft Delete / 휴지통: 페이지 삭제 시 `deletedAt` 설정, 복원(restore), 관리자 전체 비우기
- 페이지 복제(Clone): 원클릭 복제 (제목/내용/블록/태그 복사)
- 일괄 작업 API: 페이지 일괄 이동, 태그 추가/제거, 삭제
- 대시보드 통계 API: 페이지/태스크/댓글 수, 완료율, 주간 성장률
- 헬스 체크 DB 연결 검증 추가
