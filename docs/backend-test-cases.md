...
| **TC-AUTH-004** | `POST /api/auth/login`    | Log in a user successfully with correct credentials.      | User is registered and has a password. | 1. Send POST request with correct `email` and `password`.                                                         | 1. HTTP status 200. <br> 2. The server returns an HttpOnly Set-Cookie header for authentication, along with user info in the response body. | Pass   |
...
| **TC-AUTH-007** | `GET /api/auth/me`        | Get current user's info with a valid token.               | User is logged in.                     | 1. Send GET request. Authentication is strictly handled via HttpOnly Cookies — no `Authorization` header is used. | 1. HTTP status 200. <br> 2. Response body contains the user's `id`, `name`, and `email`.                                                    | Pass   |
...
#### Comment Author Display Name Fallback Rules

> **Priority:** `authorUserId` is the source of truth for resolving the display name. The storage layer performs a `LEFT JOIN` on the users table using `authorUserId` to obtain the current name at read time.
>
> **Fallback rules (in priority order):**
>
> 1. If `authorUserId` is set and the user **exists and is active** → display the user's **current name** (reflects any profile renames).
> 2. If `authorUserId` is set but the user is **inactive or withdrawn** → display **`'Unknown User'`** (account is deactivated; `authorUserId` is still stored).
> 3. If `authorUserId` is `null` (user was hard-deleted, `ON DELETE SET NULL`) → display **`'Unknown User'`**.

| Test Case ID      | API Endpoint                      | Description                                                                   | Pre-conditions                                                  | Steps                                                                         | Expected Result                                                                                                  | Status |
| :---------------- | :-------------------------------- | :---------------------------------------------------------------------------- | :-------------------------------------------------------------- | :---------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------- | :----- |
| **TC-CMT-P4-001** | `GET /api/pages/:pageId/comments` | Returns the current user name when the author has updated their display name. | Comment with `authorUserId` pointing to an active user exists.  | 1. User changes their display name. <br> 2. Fetch comments for the page.      | 1. HTTP status 200. <br> 2. `author` field reflects the **updated** name, not the name at comment-creation time. | Pass   |
| **TC-CMT-P4-002** | `GET /api/pages/:pageId/comments` | Returns `'Unknown User'` when the comment author has been deleted.            | Comment's `authorUserId` is `null` (user hard-deleted).         | 1. Fetch comments for the page.                                               | 1. HTTP status 200. <br> 2. `author` is `'Unknown User'`. <br> 3. `authorUserId` is `null`.                      | Pass   |
| **TC-CMT-P4-003** | `GET /api/pages/:pageId/comments` | Returns `'Unknown User'` when the comment author is inactive/withdrawn.       | Comment has a valid `authorUserId` but the account is inactive. | 1. Deactivate/withdraw the user account. <br> 2. Fetch comments for the page. | 1. HTTP status 200. <br> 2. `author` is `'Unknown User'`. <br> 3. `authorUserId` retains the original user ID.   | Pass   |
