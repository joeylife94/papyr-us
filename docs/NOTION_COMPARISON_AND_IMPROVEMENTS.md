...
| 지식 그래프                      | ❌        | ✅          | Papyr.us 우위                                      | -        |
| AI Copilot (채팅)                | ✅        | ✅          | 완료                                               | -        |
| **인라인 AI** (선택 텍스트 액션) | ✅        | ✅          | 완료 (요약·재작성·태스크화; `POST /api/ai/inline`) | -        |
| **AI 자동완성** (타이핑 시)      | ✅        | ⚠️ 백엔드만 | 백엔드 구현; UI 컴포넌트 미연결                    | 🟡 중간  |
| **AI 이미지 생성**               | ✅        | ❌          | 미구현                                             | 🟢 낮음  |
...
| **이메일 알림**        | ✅       | ⚠️ (SMTP nodemailer)  | SMTP 환경변수 필요; SMTP 미설정 시 숫자 사용자 ID만 인앱 알림으로 전달 (UI 입력 이메일 주소는 미지원) | -        |
...
| 21  | **이메일 알림 확장** — ~~SendGrid/Resend 연동~~ (SMTP nodemailer 이미 구현됨; SMTP 미설정 시 숫자 사용자 ID만 인앱 알림 전달; 이메일 주소 문자열 수신자 해석은 미구현) | 2일       | ⭐⭐       |
